...
async function getCarros() {
  ...
  carros.forEach((carro) => {
    ...
    const alugueisDoCarro = alugueis.filter((aluguel) => aluguel.carro === carroId);
    let x = '0';
    let y = 0;
    carro.proximasDatas=[];
    alugueisDoCarro.forEach((aluguel) => {
      const dataInicio = new Date(aluguel.dataInicio);
      const dataFinal = new Date(aluguel.dataFinal);

      if (dataAtual >= dataInicio && dataAtual <= dataFinal && aluguel.status === 'Confirmado') {
        carro.status = 'Indisponível hoje';
        x = '1';
      }
      if(dataInicio > dataAtual || dataFinal > dataAtual){
        if(y < 3 && aluguel.status === 'Confirmado'){
          carro.proximasDatas[y] = `${aluguel.dataInicio} à ${aluguel.dataFinal}`;
          y++;
        }
      }
    });

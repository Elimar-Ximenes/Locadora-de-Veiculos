...
app.get('/loja/aluguel', requireLogin, async (req, res) => {
  if(req.session.admin === "not"){
    try {
      ...
      res.render('meus-alugueis', { aluguelConfirmado, aluguelRejeitado, aluguelEmAnalise }); // Renderiza a página de aluguéis e passa os aluguéis encontrados como variável para o template
    } catch (error) {
      console.log(error);
    ...
    if (user) {
      // Define a propriedade 'user' na sessão com o username
        ...
        if (user.admin === 'yes') {
          ...
          res.send('<script>alert("Credenciais inválidas"); window.location="/login/admin";</script>');
        }
    } 
  } catch (error) {
    console.error(error);
